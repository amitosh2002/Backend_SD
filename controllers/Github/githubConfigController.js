...
export const listAllConfigs = async (req, res) => {
    try {
        const configs = await GithubConfigModel.find({});
        const maskedConfigs = configs.map(c => ({
            ...c._doc,
            githubSecretCode: '********'
        }));
        res.status(200).json({ success: true, configs: maskedConfigs });
    } catch (error) {
...
export const copyGithubConfig = async (req, res) => {
    try {
        const { sourceProjectId, targetProjectId } = req.body;
        if (!sourceProjectId || !targetProjectId) {
            return res.status(400).json({ success: false, message: "Source and Target Project IDs are required" });
        }

        const sourceConfig = await GithubConfigModel.findOne({ projectId: sourceProjectId });
        if (!sourceConfig) {
            return res.status(404).json({ success: false, message: "Source configuration not found" });
        }

        const targetConfig = await GithubConfigModel.findOneAndUpdate(
            { projectId: targetProjectId },
            {
                githubSecretCode: sourceConfig.githubSecretCode,
                updatedBy: req.user?.id
            },
            { upsert: true, new: true }
        );

        res.status(200).json({ success: true, message: "Configuration copied successfully", config: targetConfig });
    } catch (error) {
        console.error("copyGithubConfig error:", error);
        res.status(500).json({ success: false, message: "Failed to copy configuration" });
    }
};
